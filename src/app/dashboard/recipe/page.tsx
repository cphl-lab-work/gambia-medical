"use client";

import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import DashboardLayout from "@/components/DashboardLayout";
import { getStoredAuth } from "@/helpers/local-storage";
import { canRead, canCreate, canUpdate, canDelete } from "@/helpers/module-permissions";
import { MetricCard, BarChart, DashboardSection } from "@/components/dashboard/DashboardCharts";
import Link from "next/link";

const FALLBACK = {
  totalRecipes: 156,
  totalChange: 8,
  byCategory: [
    { role: "Antibiotics", count: 42 },
    { role: "Analgesics", count: 28 },
    { role: "Cardiovascular", count: 24 },
    { role: "Other", count: 62 },
  ],
};

export default function RecipePage() {
  const router = useRouter();
  const [auth, setAuth] = useState<{ role: string } | null>(null);
  const [stats, setStats] = useState<typeof FALLBACK | null>(null);

  useEffect(() => {
    const a = getStoredAuth();
    if (!a) {
      router.replace("/login");
      return;
    }
    if (!canRead(a.role, "recipe_management")) {
      router.replace("/dashboard");
      return;
    }
    setAuth({ role: a.role });
  }, [router]);

  useEffect(() => {
    if (!auth) return;
    setStats(FALLBACK);
  }, [auth]);

  const d = stats ?? FALLBACK;
  const maxBar = Math.max(...d.byCategory.map((r) => r.count), 1);
  const allowCreate = auth && canCreate(auth.role, "recipe_management");
  const allowUpdate = auth && canUpdate(auth.role, "recipe_management");
  const allowDelete = auth && canDelete(auth.role, "recipe_management");

  if (!auth) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-100">
        <p className="text-slate-500">Loading…</p>
      </div>
    );
  }

  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-slate-800">Recipe Management</h1>
            <p className="text-sm text-slate-500 mt-1">Manage drug recipes and formulations.</p>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <MetricCard title="Total recipes" value={d.totalRecipes} change={d.totalChange} icon={<span className="text-lg">📋</span>} />
          <MetricCard title="Categories" value={d.byCategory.length} change={0} icon={<span className="text-lg">📁</span>} />
        </div>

        <DashboardSection title="Recipes by category">
          <BarChart items={d.byCategory} max={maxBar} barColor="bg-amber-600" />
        </DashboardSection>

        <DashboardSection title="Content view – Recipe list">
          <p className="text-sm text-slate-500 mb-4">
            {allowCreate && "You can add recipes. "}
            {allowUpdate && "You can edit. "}
            {allowDelete && "You can delete (Admin). "}
            {!allowCreate && !allowUpdate && "You have view-only access."}
          </p>
          <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50/50 p-8 text-center text-slate-500">
            <p className="font-medium text-slate-700">Recipe list and form</p>
            <p className="text-sm mt-1">Table and create/edit form will be implemented here.</p>
          </div>
        </DashboardSection>
      </div>
    </DashboardLayout>
  );
}
